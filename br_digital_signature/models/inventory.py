...
class InventoryInherit(models.Model):
    ...
    def _default_show_sign(self):
        return self.env['ir.config_parameter'].sudo().get_param(
            'br_digital_signature.show_digital_sign_inventory')

    def _default_enable_options(self):
        return self.env['ir.config_parameter'].sudo().get_param(
            'br_digital_signature.enable_options_inventory')
    ...
    def button_validate(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param(
            'br_digital_signature.confirm_sign_inventory') and self.digital_sign is False:
    ...
    def _compute_sign_applicable(self):
        for rec in self:
            rec.sign_applicable = self.env['ir.config_parameter'].sudo().get_param(
                'br_digital_signature.sign_applicable')
